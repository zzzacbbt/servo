...
use webrender_api::{self, ClipId, SpaceAndClipInfo, DisplayListBuilder, RasterSpace, SpatialId};
...
impl WebRenderDisplayListConverter for DisplayList {
    fn convert_to_webrender(&self, pipeline_id: PipelineId) -> DisplayListBuilder {
        ...
        //builder.push_clip_id(state.active_clip_id);
        //builder.push_spatial_id(state.active_spatial_id);
...
impl WebRenderDisplayItemConverter for DisplayItem {
    ...
    fn convert_to_webrender(
        ...
        if cur_spatial_id != state.active_spatial_id {
            //builder.pop_spatial_id();
            //builder.push_spatial_id(cur_spatial_id);
            state.active_spatial_id = cur_spatial_id;
        }

        let internal_clip_id = clip_and_scroll_indices.clipping
        ...
        if cur_clip_id != state.active_clip_id {
            //builder.pop_clip_id();
            //builder.push_clip_id(cur_clip_id);
        ...
        match *self {
            ...
            DisplayItem::PushStackingContext(ref item) => {
                ...
                let spatial_id = if let Some(frame_index) = stacking_context.established_reference_frame {
                    debug_assert!(
                    ...
                    //builder.push_spatial_id(spatial_id);
                    //last_spatial_id = Some(state.active_spatial_id);
                    //state.active_spatial_id = spatial_id;
                    spatial_id
                } else {
                    state.active_spatial_id
                };
            ...
            DisplayItem::DefineClipScrollNode(ref item) => {
                ...
                match node.node_type {
                    ...
                    ClipScrollNodeType::ScrollFrame(scroll_sensitivity, external_id) => {
                        ...
                        state.spatial_ids[item.node_index.to_index()] = Some(space_clip_info.spatial_id);
                    }
                    ClipScrollNodeType::StickyFrame(ref sticky_data) => {
                        // TODO: Add define_sticky_frame_with_parent to WebRender.
                        //builder.push_spatial_id(parent_spatial_id);
                        //active_
